...
class Constants(BaseConstants):
    name_in_url = 'group_decision_info'
    players_per_group = 3
    num_rounds = 1
        ...
        for player in players:
            if self.audited:
                if Constants.company_income == self.total_income_entered:
                    player.payoff = (((Constants.payoff_multiplier * (
                                Constants.company_income - tax_payment)) / Constants.divisor) + Constants.show_up_fee)
                else:
                    player.payoff = ((Constants.payoff_multiplier * (Constants.company_income - tax_payment - (
                                2 * unpaid_tax)) / Constants.divisor) + Constants.show_up_fee)
            else:
                player.payoff = (((Constants.payoff_multiplier * (
                            Constants.company_income - tax_payment)) / Constants.divisor) + Constants.show_up_fee)
...
class Player(BasePlayer):
    ...
    def chat_nickname(self):
        if self.participant.vars['gender'] == 'Female':
            return 'female participant'
        else:
            return 'male participant'
